...
// -------------------------
//
export async function getLessonPlans(page: number = 1, limit: number = 10, query?: string) {
    ...
    if (trimmed) {
        ...
        q = q.or(
            `title.ilike.%${safe}%,subject.ilike.%${safe}%,topic.ilike.%${safe}%,sub_topic.ilike.%${safe}%,class_level.ilike.%${safe}%`
        );
    }
